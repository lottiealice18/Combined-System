...
def process_and_display(df, column_name, title):
    if len(df) > 0:
        ...
        top_horses = top_horses[['Time', 'Venue', 'Date', column_name, 'Race Type', 'Back Odd', 'Trainer', 'Horse']]

        if len(top_horses) > 0:
            top_horses['Time'] = top_horses['Time'].astype(str)
            top_horses = top_horses.sort_values('Time')
            top_horses = top_horses[['Horse', 'Time', 'Venue', 'Date', column_name, 'Race Type', 'Back Odd', 'Jockey', 'Trainer']]
